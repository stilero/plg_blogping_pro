 ...
 * 
 * $Id: jArticle.php 19 2012-09-01 14:29:48Z webbochsant@gmail.com $
...
class BPArticle {
    ...
    public function __construct($article) {
        ...
        $tempClass->published = $this->isPublished($article);
    ...
    public function imagesInContent($article){
        ...
        if($content=='' && isset($article->fulltext)){
            $content = $article->fulltext;    
        }
        if($content=='' && isset($article->introtext)){
    ...
    public function description($article){
        ...
        if(isset($article->introtext) && $article->introtext!=""){
            $descText = $description = $article->introtext;
        }elseif (isset($article->metadesc) && $article->metadesc!="" ) {
            $descText = $description = $article->metadesc;
        }
        $descNeedles = array("\n", "\r", "\"", "'");
 ...
 * For K2 items
 */
class BPk2Article extends BPArticle{
    public function __construct($article) {
 ...
 * For Zoo articles
 */
class BPzooArticle extends BPArticle{
    public function __construct($article) {
        parent::__construct($article);
    }
}
 ...
 * For VirtueMart
 */
class BPvmArticle extends BPArticle{
